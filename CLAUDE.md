...
A projekt átmeneti állapotban van: a térkép, a tudástár és a monitoring még a `lib/data.ts` / `lib/knowledge.ts` demóadataiból dolgozik; a riportok (információs bejegyzések), a kommunikáció (ticketek) és a felhasználók már DB-ből. A cél a demóadatok fokozatos kiváltása. Terv és spec: `docs/superpowers/`.
...
| Megosztott komponensek | `components/<terület>/` | Több route-on használt, saját komponensek, alkönyvtárba rendezve (pl. `components/riport/`, `components/form/`: `useMuveletForm`, `MezoHiba`, `MuveletDialog`). A shadcn primitívekből épülnek. Az `AppShell.tsx` és a `WorldMap.tsx` már itt van. |
